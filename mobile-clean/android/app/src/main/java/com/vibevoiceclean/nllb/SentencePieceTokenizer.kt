...
class SentencePieceTokenizer {
  ...
  fun load(path: String) {
    if (!File(path).exists()) throw IllegalArgumentException("SentencePiece model not found at $path")
    modelPath = path
    tokenToId.clear()
    idToToken.clear()
    nextDynamicId = 4
  ...
  fun encodeToIds(text: String): LongArray {
    ...
    ids[0] = bosId.toLong()
    pieces.forEachIndexed { index, token -> ids[index + 1] = resolveTokenId(token).toLong() }
  ...
  fun decodeFromIds(tokenIds: List<Int>): String {
    ensureLoaded()
    return tokenIds.filter { it != bosId && it != eosId && it != padId }
  ...
  private fun resolveTokenId(token: String): Int = tokenToId[token] ?: run {
